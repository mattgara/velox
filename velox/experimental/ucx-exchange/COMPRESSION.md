...
| --- | --- | --- |
| `cudf.exchange_compression` | `none` | `none` sends raw data. `column` tries per-column compression. |
...
```properties
cudf.exchange=true
cudf.exchange_compression=column
...
This layer intentionally has no runtime profitability model. A later layer can
choose dynamically between `column` and raw transfer.
...
With `VELOX_BUILD_TESTING=ON`, build the focused test target:
