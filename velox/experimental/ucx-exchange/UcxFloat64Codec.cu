...
Float64CompressResult compressFloat64(
  ...
  rmm::device_buffer planes(
      static_cast<std::size_t>(kFloat64Bytes) * stride, stream);
  UCX_FLOAT64_CUDA_CHECK(
      cudaMemsetAsync(planes.data(), 0, planes.size(), stream.value()));
  ...
  rmm::device_buffer compressedScratch(
      static_cast<std::size_t>(exponentPlanes) * maxCompressed, stream);
  rmm::device_buffer sizesDevice(exponentPlanes * sizeof(uint32_t), stream);
  ...
  for (uint32_t plane = exponentPlanes; plane < kFloat64Bytes; ++plane) {
    UCX_FLOAT64_CUDA_CHECK(cudaMemcpyAsync(
        static_cast<uint8_t*>(result.data.data()) + outputOffset,
