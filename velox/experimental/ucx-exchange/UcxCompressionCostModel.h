...
class UcxCompressionCostModel {
 public:
  ...
  /// Records one normal codec invocation. `encodedBytes` is the size that the
  /// codec path would send, which equals `rawBytes` when compression is
  /// rejected.
  void recordEncode(
 ...
 private:
  ...
  StageSamples& getOrCreateStage(std::string_view taskId);
  ...
  std::deque<std::string> stageOrder_;
};
