...
DecimalSumStateColumns deserializeDecimalSumStateWithCount(
  ...
  // For fully-null state columns there is nothing to deserialize. Avoid launching
  // unpack kernels over string payload buffers that may be empty for this case.
...
std::unique_ptr<cudf::column> serializeDecimalSumState(
  ...
  if (useLargeOffsets) {
    fillOffsetsKernel<int64_t><<<offsetGridSize, blockSize, 0, stream.value()>>>(
        offsetsView.data<int64_t>(), numRows);
  } else {
    fillOffsetsKernel<int32_t><<<offsetGridSize, blockSize, 0, stream.value()>>>(
  ...
  if (numRows > 0) {
    ...
    if (useLargeOffsets) {
      ...
      if (sumCol.type().id() == cudf::type_id::DECIMAL64) {
        packStateKernel<int64_t, int64_t><<<gridSize, blockSize, 0, stream.value()>>>(
            sumCol.data<int64_t>(),
        ...
        packStateKernel<__int128_t, int64_t><<<gridSize, blockSize, 0, stream.value()>>>(
      ...
      if (sumCol.type().id() == cudf::type_id::DECIMAL64) {
        packStateKernel<int64_t, int32_t><<<gridSize, blockSize, 0, stream.value()>>>(
            sumCol.data<int64_t>(),
            countCol.data<int64_t>(),
            offsetsPtr,
            charsPtr,
        ...
        CUDF_EXPECTS(
            sumCol.type().id() == cudf::type_id::DECIMAL128,
            "Unsupported decimal sum column type");
        packStateKernel<__int128_t, int32_t><<<gridSize, blockSize, 0, stream.value()>>>(
            sumCol.data<__int128_t>(),
            countCol.data<int64_t>(),
            offsetsPtr,
            charsPtr,
