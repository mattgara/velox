...
TpchPlan TpchQueryBuilder::getQ15Plan() const {
  // SIMPLIFIED Q15: Just TableScan for race condition debugging
  // This eliminates all complex query processing and focuses purely on cuDF parquet reading
  std::vector<std::string> lineitemColumns = {
  ...
  // SIMPLE PLAN: Just scan lineitem table with filter and exit
  // This isolates the cuDF parquet reading race condition without query complexity
  auto plan =
      PlanBuilder(planNodeIdGenerator, pool_.get())
          .filtersAsNode(filtersAsNode_)
          .tableScan(
